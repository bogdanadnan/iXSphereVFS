...
### What
`vfs_open`, `vfs_readdir`, `vfs_file_size`, and `vfs_file_mtime` use
...
In `vfs_open` (tree.c:867), `vfs_readdir`, and `vfs_file_size`:
```c
...
- **After commit (1→2):** Reading at epoch 1 — not remapped to 2. Entries at
  ...
  `vfs_open` returns VFS_ERR_NOTFOUND for committed files.
...
|------|----------|------------|
| `src/tree.c` | `vfs_open` | DirContent |
...
### Acceptance
- [ ] After commit: `vfs_open` at snapshot epoch finds committed files
- [ ] After soft-delete: `vfs_open` at deleted epoch returns VFS_ERR_NOTFOUND
