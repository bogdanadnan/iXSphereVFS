...
int dentry_cache_build(Pool* pool, MapperTable* mapper_table, int64_t root_vp, int64_t epoch,
    ...
    while (walk_vp != 0 && best_count < DENTRY_CACHE_MAX) {
        ...
        if (mapper_table_traversal_apply(mapper_table, (int64_t)ce_epoch))
            effective_epoch = mapper_table_resolve(mapper_table, (int64_t)ce_epoch);
    ...
    for (int i = 0; i < best_count && arr->count < DENTRY_CACHE_MAX; i++) {
        ...
        while (walk_vp != 0) {
            ...
            if (mapper_table_traversal_apply(mapper_table, (int64_t)dc_epoch))
                dc_eff = mapper_table_resolve(mapper_table, (int64_t)dc_epoch);
